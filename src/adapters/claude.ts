...
export class ClaudeAdapter extends BaseAdapter {
  ...
  private async hasCc(): Promise<boolean> {
    if (this.ccAvailable === undefined) {
      ...
      this.ccAvailable = result.exitCode === 0 && result.stdout.includes('claude');
    }
    return this.ccAvailable;
  }

  async getInteractiveLaunchCommand(options: LaunchOptions): Promise<ShellCommand> {
  ...
  protected buildNonInteractiveArgs(prompt: string, options?: AskOptions): string[] {
    const args = ['-p', prompt];
  ...
  async registerMcpServer(name: string, command: string, args: string[]): Promise<void> {
    const mcpArgs = ['mcp', 'add', '-s', 'project', name, '--', command, ...args];
    ...
    if (result.exitCode !== 0) {
      throw new Error(`Failed to register MCP server "${name}": ${result.stderr}`);
    }
