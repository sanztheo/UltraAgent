 ...
 *
 * Each worker can operate in its own worktree for isolation.
 * Supports detached HEAD and named branches, with rollback on failure.
 */
...
const BRANCH_IN_USE_PATTERN =
  /already checked out|already used by worktree|is already checked out/i;
...
function resolveWorktreePath(
  ...
  const bucket = `${basename(repoRoot)}.ua-worktrees`;
  ...
  return join(parent, bucket, `team-${workerName}`);
}
...
function hasBranchInUse(
  ...
  return entries.some(
    (entry) =>
      entry.branchRef === expectedRef && resolve(entry.path) !== resolvedPath,
  );
}
...
function readWorktreeEntryFromPath(
  ...
  const branchRef =
    branchResult.status === 0 ? (branchResult.stdout || "").trim() : null;
...
export function planWorktreeTarget(
  input: WorktreePlanInput,
): PlannedWorktreeTarget | { enabled: false } {
...
export function ensureWorktree(
  ...
  const expectedBranchRef = plan.branchName
  ...
  if (
    plan.branchName &&
    hasBranchInUse(allWorktrees, plan.branchName, plan.worktreePath)
  ) {
    throw new Error(`branch_in_use:${plan.branchName}`);
  }

  mkdirSync(dirname(plan.worktreePath), { recursive: true });
  const branchAlreadyExisted = plan.branchName
    ? branchExists(plan.repoRoot, plan.branchName)
    : false;
...
export async function rollbackProvisionedWorktrees(
  ...
  const created = results
    .filter(
      (result): result is EnsureWorktreeResult =>
        result.enabled === true && result.created,
    )
    .reverse();
  ...
  for (const result of created) {
    try {
      await execFileAsync(
        ...
        { cwd: result.repoRoot, encoding: "utf-8" },
      );
    } catch (err: unknown) {
      const stderr = (
      ...
      errors.push(
        `remove:${result.worktreePath}:${stderr || `exit_${exitCode}`}`,
      );
      continue;
    }
    ...
    try {
      await execFileAsync("git", ["branch", "-D", result.branchName], {
        cwd: result.repoRoot,
        encoding: "utf-8",
      });
    } catch (err: unknown) {
      if (branchExists(result.repoRoot, result.branchName)) {
        ...
        const exitCode = (err as Record<string, unknown>).code;
        errors.push(
          `delete_branch:${result.branchName}:${stderr || `exit_${exitCode}`}`,
        );
      }
    }
  }
