...
export async function initCommand(): Promise<void> {
  ...
  if (detected.length === 0) {
    p.cancel('No AI CLIs found. Install at least one of: claude, codex, gemini');
    process.exit(1);
  }

  p.note(detected.map((d) => `  ${chalk.green('✓')} ${d.name} (${d.path})`).join('\n'), 'Detected CLIs');
  ...
  p.outro(chalk.green(`UltraAgent configured! Run ${chalk.bold('ultraagent')} to start.`));
}
